...
class Predictor:
    ...
    def run(self, imgs_path):
        ...
        # inference
        data = np.array([self._preprocess(imgs_path)])
        input_handle.reshape(data.shape)
        input_handle.copy_from_cpu(data)
        self.predictor.run()
        # results[]是一个numpy类型的二维数组
        results = output_handle.copy_to_cpu()
        results = self._postprocess(results)

        # 显示图片
        img, ret = self._showImgRres(results, imgs_path)
        # print(imgs_path)
        logger.info("Finish")
        return img, ret
    ...
    def _showImgRres(self, result, img):
        # 输出结果为二值化二阶矩阵
        result = result * 255  # 二值化色彩排布
