...
class TextRecognizer(object):
    def __init__(self, args):
        ...
        self.dict = open(args.rec_char_dict_path, "r").read().split("\n")
        postprocess_params = {
            'name': 'CTCLabelDecode',
    ...
    def __call__(self, img_list):
        ...
        width_list = []
        width_list.append(img_list.shape[1] / float(img_list.shape[0]))
        ...
        # max_wh_ratio = 0
        h, w = img_list.shape[0:2]
        wh_ratio = w * 1.0 / h
        max_wh_ratio = max(max_wh_ratio, wh_ratio)
        norm_img = self.resize_norm_img(img_list, max_wh_ratio)
        norm_img = norm_img[np.newaxis, :]
        ...
        # 去0值
        preds_prob = preds_prob[preds_idx != 0]
        preds_idx = preds_idx[preds_idx != 0]
        # 从字典中取值
        res = [self.dict[x - 1] for x in preds_idx]
        # print(res)
        # rec_result = ''.join(res)
        # =========================
        # 这里是将输出的预测值转换为字典中的值
...
class start_rec:
    def __init__(self, model=None, dic=None):
        ...
        # self.args.rec_char_dict_path = None

    def start(self, pic):
        rec_res = None
        self.args.image_dir = pic
        # image_file_list = get_image_file_list(self.args.image_dir)
        text_recognizer = TextRecognizer(self.args)
        # warmup 2 times
        ...
        #     logger.info("Predicts res:{}".format(rec_res[ino]))
